...
const Button = (props) => {
  ...
  return <button onClick={klikkaus}>{teksti}</button>;
};
  ...
  return (
    <table>
      <tbody>
        <tr>
          <td>
            {props.text} {props.value} {props.merkki}
          </td>
        </tr>
      </tbody>
    </table>
...
const App = () => {
  ...
  return (
    <div>
      ...
      <Anecdotes
        teksti={anecdotes[valittu]}
        click={randomAnacdote()}
        vote={pisteet(numero)}
        numero={numero[valittu]}
      />
      <Display
        teksti={"Anecdote with most votes"}
        eniten={eniten}
        voittaja={suosituinSana}
      />
    </div>
  );
};
